...
class GSCClient:
    ...
    def list_properties(self):
        ...
        return [{"siteUrl": i["siteUrl"], "permissionLevel": i["permissionLevel"]} for i in result.get("siteEntry", [])]
    ...
    def get_performance(self, site_url, page_url, days=7):
        # Tarih Aralığı: Bugün ve 'days' kadar öncesi
        ...
        body = {
            ...
            "dataState": "all" # En güncel (kesinleşmemiş) verileri dahil et
        }
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(APP_TITLE); self.resize(1280, 850)
    ...
    def build_ui(self):
        ...
        title_v.addWidget(QLabel("URL İndeks ve Performans Takip Sistemi", objectName="SubTitle"))
        
        self.login_btn = QPushButton("Google Bağla"); self.login_btn.clicked.connect(self.handle_login)
        self.time_combo = QComboBox(); self.time_combo.addItems(["Son 24 Saat", "Son 7 Gün", "Son 28 Gün", "Son 3 Ay"])
        ...
        self.prop_combo = QComboBox(); i_lay.addWidget(QLabel("Mülk Seçimi:")); i_lay.addWidget(self.prop_combo)
        self.url_text = QTextEdit(); self.url_text.setPlaceholderText("URL'leri buraya yapıştırın..."); i_lay.addWidget(self.url_text)
    ...
    def apply_styles(self):
        self.setStyleSheet("""
            QMainWindow, QWidget { background-color: #0F0F0F; color: #EEE; font-family: sans-serif; }
            QFrame#Card, QFrame#TopPanel { background-color: #1A1A1A; border: 1px solid #333; border-radius: 10px; padding: 10px; }
            QFrame#StatCard { background-color: #222; border-radius: 8px; }
            QLabel#MainTitle { font-size: 20px; font-weight: bold; color: #D4AF37; }
            QLabel#StatValue { font-size: 24px; font-weight: bold; }
            QPushButton { background-color: #D4AF37; color: #000; font-weight: bold; border-radius: 5px; padding: 8px 15px; }
            QPushButton#ActionBtn { background-color: #2E7D32; color: white; min-width: 200px; }
            QTextEdit, QComboBox { background-color: #252525; border: 1px solid #444; border-radius: 4px; padding: 5px; color: #FFF; }
            QTableWidget { background-color: #1A1A1A; gridline-color: #333; }
            QHeaderView::section { background-color: #222; color: #D4AF37; font-weight: bold; border: none; padding: 5px; }
        """)
    ...
    def handle_login(self):
        try:
            ...
            for p in props: self.prop_combo.addItem(p["siteUrl"], p["siteUrl"])
            self.check_btn.setEnabled(True)
            QMessageBox.information(self, "Başarılı", "GSC Bağlantısı kuruldu.")
    ...
    def start_check(self):
        ...
        self.worker = InspectionWorker(self.gsc, self.prop_combo.currentData(), urls, days)
    ...
    def add_row(self, d):
        ...
        self.table.setItem(r, 0, QTableWidgetItem(d["url"]))
    ...
    def finish_ui(self, s):
        ...
        # Toplam tıklama ve gösterimi hesapla
        ...
        self.check_btn.setEnabled(True)
        QMessageBox.information(self, "Tamamlandı", "Analiz başarıyla bitti.")
